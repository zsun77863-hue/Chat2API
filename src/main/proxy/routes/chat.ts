...
router.post('/completions', async (ctx: Context) => {
  ...
  if (agentLoopEnabled) {
    // ===== AGENT LOOP PATH (v1.4.1) =====
    // Handles multi-turn tool-calling automatically with session caching.
    // Includes graceful degradation: if agent loop fails, falls back to single-shot.
    try {
      ...
      const agentContext: ProxyContext = {
        ...
        actualModel,
        startTime,
        isStream: false, // Agent loop always uses non-stream for reliable parsing
        clientIP,
      }

      proxyStatusManager.recordRequestStart(request.model, provider.id, account.id)

      const { result: agentResult, sessionId, waitingForTools } = await agentLoopManager.handleRequest(
        request,
        account,
        provider,
        actualModel,
      ...
      if (!agentResult.success) {
        ...
        // Bug 4: Graceful degradation — if agent loop fails, try normal single-shot
        console.warn(`[Chat] Agent loop failed (${agentResult.error}), attempting graceful degradation`)
        try {
          const fallbackResult = await requestForwarder.forwardChatCompletion(
            request,
            account,
            provider,
            actualModel,
          ...
          if (fallbackResult.success) {
            ...
            ctx.set('X-Agent-Fallback', 'true')
            ctx.body = fallbackResult.body || {
              id: requestId,
            ...
            storeManager.recordRequestInStats(true, Date.now() - startTime, request.model, provider.id, account.id)
            return
          }
        } catch (fallbackError) {
      ...
      // Signal to client that we're waiting for tool results
      if (waitingForTools) {
        ctx.set('X-Agent-Waiting-For-Tools', 'true')
      }
      ...
      // Bug 4: Graceful degradation on unexpected exceptions
      console.error('[Chat] Agent loop exception, attempting fallback:', error)
      try {
        const config = storeManager.getConfig()
        const preferredProviderId = modelMapper.getPreferredProvider(request.model)
        const preferredAccountId = modelMapper.getPreferredAccount(request.model)
        ...
        if (selection) {
          const { account, provider, actualModel } = selection
          const fallbackContext: ProxyContext = {
            requestId,
            providerId: provider.id,
            accountId: account.id,
            model: request.model,
          ...
          const fallbackResult = await requestForwarder.forwardChatCompletion(
            request, account, provider, actualModel, fallbackContext
          )
          if (fallbackResult.success) {
            ctx.set('Content-Type', 'application/json')
            ctx.set('X-Agent-Fallback', 'true')
